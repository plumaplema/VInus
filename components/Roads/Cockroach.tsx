...
const Cockroach: React.FC<BigEyeProps> = () => {
    ...
    const data: any = cockroachRoadCompilation.map((data) => data.map((content) => content === '⚪' ? <View style={{ backgroundColor: 'white', width: 2, height: '90%', transform: 'rotate(45deg)', margin: 4 }} /> : content == '🔴' ? <View style={{ backgroundColor: 'red', width: 2, height: '90%', transform: 'rotate(45deg)', margin: 4 }} /> : <View style={{ backgroundColor: 'blue', width: 2, height: '90%', transform: 'rotate(45deg)', margin: 4 }} />))
    return (
